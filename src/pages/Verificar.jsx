  ...
  return (
    <div style={{ maxWidth: 640, margin: '0 auto', padding: '100px 24px 60px' }}>
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--b-neutral)', borderRadius: 'var(--r-xl)', padding: 28, marginBottom: 24 }}>
        ...
        {error && <p style={{ color: 'var(--cr5)', fontSize: '.82rem', marginBottom: 12 }}>{error}</p>}
      ...
      {result && (
        <div style={{ background: 'var(--surface)', border: `1px solid ${result.valid ? 'rgba(16,185,129,0.3)' : 'rgba(185,28,28,0.3)'}`,
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 20 }}>
            ...
            <div>
              <div className="t-display" style={{ fontSize: '1.3rem', fontWeight: 700, color: result.valid ? 'var(--jade2)' : 'var(--cr4)' }}>
                {result.valid ? 'Relatório Autêntico' : 'Relatório Inválido'}
              </div>
              <div style={{ fontSize: '.8rem', color: 'var(--p4)', fontFamily: 'var(--f-mono)', marginTop: 2 }}>{result.serial}</div>
            </div>
          </div>
          {result.valid && (
          ...
          {result.aviso && <p style={{ fontSize: '.78rem', color: 'var(--p5)', marginTop: 16, fontStyle: 'italic', lineHeight: 1.6 }}>⚠ {result.aviso}</p>}
        </div>
      )}
    </div>
  );
