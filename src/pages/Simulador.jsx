...
export default function SimuladorPage() {
  ...
  const pctColor = (p) => p >= 65 ? 'var(--emerald2)' : p >= 40 ? 'var(--g4)' : 'var(--r3)';

  if (!authToken) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 16 }}>
      <Scale size={32} style={{ color: 'var(--n5)' }}/>
      <p style={{ color: 'var(--n4)' }}>Faça login para usar o simulador.</p>
  ...
  return (
    <div style={{ maxWidth: 860, margin: '0 auto', padding: '100px 24px 60px' }}>
      <div style={{ marginBottom: 36 }}>
        ...
        <p style={{ color: 'var(--n4)', fontSize: '.9rem' }}>Estima probabilidades de êxito em cada instância judiciária.</p>
      </div>
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--bn)', borderRadius: 'var(--r-xl)', padding: 28, marginBottom: 24 }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
          ...
          <div>
            <label style={{ display: 'block', fontSize: '.74rem', color: 'var(--n4)', fontFamily: 'var(--f-mono)', marginBottom: 8 }}>ÁREA DO DIREITO</label>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            {INSTANCES.map(({ key, label }) => {
              ...
              return (
                <div key={key} style={{ background: 'var(--surface)', border: `1px solid ${color}30`, borderRadius: 'var(--r-md)', padding: '18px 16px' }}>
                  <div style={{ fontSize: '.72rem', color: 'var(--n4)', fontFamily: 'var(--f-mono)', letterSpacing: '.06em', marginBottom: 8 }}>{label}</div>
                  <div style={{ fontFamily: 'var(--f-display)', fontSize: '2rem', fontWeight: 700, color, marginBottom: 10 }}>{pct.toFixed(0)}%</div>
                  <div style={{ height: 6, background: 'var(--bn2)', borderRadius: 'var(--r-pill)', overflow: 'hidden' }}>
          ...
          {result.estrategia_recomendada && (
            <div style={{ background: 'var(--surface)', border: '1px solid var(--bg)', borderRadius: 'var(--r-md)', padding: 20 }}>
              <div style={{ fontSize: '.72rem', color: 'var(--g4)', fontFamily: 'var(--f-mono)', letterSpacing: '.08em', marginBottom: 10 }}>ESTRATÉGIA RECOMENDADA</div>
              <p style={{ fontSize: '.88rem', color: 'var(--n2)', lineHeight: 1.7 }}>{result.estrategia_recomendada}</p>
            </div>
          )}
          ...
          {result.fatores_positivos?.length > 0 && (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              <div style={{ background: 'var(--surface)', border: '1px solid rgba(16,185,129,0.2)', borderRadius: 'var(--r-md)', padding: 18 }}>
                <div style={{ fontSize: '.72rem', color: 'var(--emerald2)', fontFamily: 'var(--f-mono)', marginBottom: 10 }}>✓ FATORES POSITIVOS</div>
                <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {result.fatores_positivos.map((f, i) => <li key={i} style={{ fontSize: '.83rem', color: 'var(--n3)' }}>• {f}</li>)}
                </ul>
              </div>
              {result.fatores_negativos?.length > 0 && (
                <div style={{ background: 'var(--surface)', border: '1px solid rgba(185,28,28,0.2)', borderRadius: 'var(--r-md)', padding: 18 }}>
                  <div style={{ fontSize: '.72rem', color: 'var(--r3)', fontFamily: 'var(--f-mono)', marginBottom: 10 }}>✗ FATORES DE RISCO</div>
                  <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 6 }}>
                    {result.fatores_negativos.map((f, i) => <li key={i} style={{ fontSize: '.83rem', color: 'var(--n4)' }}>• {f}</li>)}
                  </ul>
                </div>
              )}
            </div>
          ...
          {result.observacoes && (
            <div style={{ background: 'var(--surface)', border: '1px solid var(--bn)', borderRadius: 'var(--r-md)', padding: 18 }}>
              <div style={{ fontSize: '.72rem', color: 'var(--n4)', fontFamily: 'var(--f-mono)', marginBottom: 8 }}>OBSERVAÇÕES</div>
              <p style={{ fontSize: '.85rem', color: 'var(--n3)', lineHeight: 1.7 }}>{result.observacoes}</p>
            </div>
          )}
        </div>
      )}
