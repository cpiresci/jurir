...
export default function MonitoramentoPage() {
  ...
  if (!authToken) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 16 }}>
      <Bell size={32} style={{ color: 'var(--n5)' }}/>
      <p style={{ color: 'var(--n4)' }}>Faça login para monitorar processos.</p>
  ...
  return (
    <div style={{ maxWidth: 860, margin: '0 auto', padding: '100px 24px 60px' }}>
      <div style={{ marginBottom: 36 }}>
        ...
        <p style={{ color: 'var(--n4)', fontSize: '.9rem' }}>Acompanhe movimentações via DATAJUD com atualizações automáticas diárias.</p>
      </div>

      <div style={{ background: 'var(--surface)', border: '1px solid var(--bn)', borderRadius: 'var(--r-xl)', padding: 24, marginBottom: 24 }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr auto auto', gap: 12, alignItems: 'end' }}>
          <div>
            <label style={{ display: 'block', fontSize: '.74rem', color: 'var(--n4)', fontFamily: 'var(--f-mono)', marginBottom: 8 }}>NÚMERO DO PROCESSO (CNJ)</label>
          ...
          <div>
            <label style={{ display: 'block', fontSize: '.74rem', color: 'var(--n4)', fontFamily: 'var(--f-mono)', marginBottom: 8 }}>TRIBUNAL</label>
          ...
          <button className="btn btn-flame" onClick={handleAdd} disabled={adding} style={{ alignSelf: 'flex-end' }}>
      ...
      {loading ? (
        <div style={{ textAlign: 'center', color: 'var(--n5)', padding: 40 }}>Carregando…</div>
      ) : list.length === 0 ? (
        <div style={{ background: 'var(--surface)', border: '1px solid var(--bn)', borderRadius: 'var(--r-lg)', padding: 48, textAlign: 'center' }}>
          <Bell size={32} style={{ color: 'var(--n5)', marginBottom: 12 }}/>
          <p style={{ color: 'var(--n4)' }}>Nenhum processo em monitoramento.</p>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {list.map(p => (
            <div key={p.numero_processo} style={{ background: 'var(--surface)', border: '1px solid var(--bn)',
              borderRadius: 'var(--r-md)', padding: '16px 20px', display: 'flex', alignItems: 'flex-start', gap: 16 }}>
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 6, flexWrap: 'wrap' }}>
                  <span style={{ fontFamily: 'var(--f-mono)', fontSize: '.88rem', color: 'var(--n1)', fontWeight: 600 }}>{p.numero_processo}</span>
                  <span style={{ fontSize: '.7rem', background: 'var(--lift)', border: '1px solid var(--bn)', borderRadius: 4, padding: '1px 8px', color: 'var(--n4)', fontFamily: 'var(--f-mono)' }}>{p.tribunal}</span>
                  {p.score_atual != null && <span style={{ fontSize: '.7rem', color: 'var(--g4)', fontFamily: 'var(--f-mono)' }}>Score: {p.score_atual}</span>}
                </div>
                {p.ultima_mov && <div style={{ fontSize: '.82rem', color: 'var(--n3)', marginBottom: 4 }}>Última mov: {p.ultima_mov}</div>}
              ...
              <button className="btn btn-ghost btn-sm" onClick={() => handleRemove(p.numero_processo)}
                ...
                style={{ borderColor: 'rgba(185,28,28,0.2)', color: 'var(--r4)' }}>
