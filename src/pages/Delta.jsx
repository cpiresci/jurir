    ...
    <div style={{ maxWidth: 1000, margin: '0 auto', padding: '100px 24px 60px' }}>
        ...
        {[
          ...
          <div key={label}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
              <label style={{ fontSize: 'var(--fs-xs)', color: 'var(--p4)', fontFamily: 'var(--f-mono)' }}>
                {label}
              </label>
              ...
              <button type="button" className="btn btn-ghost btn-sm" disabled={attaching[slot]}
                ...
                style={{ fontSize: 'var(--fs-xs)', padding: '4px 10px', whiteSpace: 'nowrap' }}>
            ...
            <textarea value={value} onChange={e => set(e.target.value)} placeholder={placeholder}
              style={{ width: '100%', minHeight: 280, resize: 'vertical',
                ...
                fontSize: 'var(--fs-base)', lineHeight: 1.6, padding: '14px 16px', outline: 'none' }}
      ...
      <div style={{ display: 'flex', gap: 12, alignItems: 'center', marginBottom: 32, flexWrap: 'wrap' }}>
        <select value={lang} onChange={e => setLang(e.target.value)}
          style={{ background: 'var(--ridge)', color: 'var(--p2)', border: '1px solid var(--b-neutral)',
            borderRadius: 'var(--r-sm)', padding: '9px 12px', fontFamily: 'var(--f-sans)', fontSize: 'var(--fs-sm)', outline: 'none' }}>
      ...
      {result && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          {result.doc_type && (
            <div style={{ fontSize: 'var(--fs-xs)', fontFamily: 'var(--f-mono)', color: 'var(--au6)',
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(160px,1fr))', gap: 12 }}>
            {[
              ...
              <div key={label} style={{ background: 'var(--surface)', border: '1px solid var(--b-neutral)', borderRadius: 'var(--r-md)', padding: '14px 16px' }}>
                <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--p5)', fontFamily: 'var(--f-mono)', letterSpacing: '.08em', marginBottom: 6 }}>{label}</div>
                <div style={{ fontSize: 'var(--fs-2xl)', fontWeight: 700, fontFamily: 'var(--f-display)', color }}>{val}</div>
              </div>
            ))}
          </div>
          ...
          {result.semantic_impact && (
            <div style={{ background: 'var(--surface)', border: '1px solid var(--b-neutral)', borderRadius: 'var(--r-md)', padding: 20 }}>
              <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--au6)', fontFamily: 'var(--f-mono)', letterSpacing: '.08em', marginBottom: 12 }}>ANÁLISE SEMÂNTICA</div>
              {result.semantic_impact.summary && (
                <p style={{ fontSize: 'var(--fs-base)', color: 'var(--p2)', lineHeight: 1.7, marginBottom: 12 }}>{result.semantic_impact.summary}</p>
              )}
              {result.semantic_impact.recommendations?.length > 0 && (
                <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {result.semantic_impact.recommendations.map((rec, i) => (
                    <li key={i} style={{ display: 'flex', gap: 10, fontSize: 'var(--fs-sm)', color: 'var(--p3)' }}>
          ...
          {result.blocks?.length > 0 && (
            <div style={{ background: 'var(--surface)', border: '1px solid var(--b-neutral)', borderRadius: 'var(--r-md)', padding: 20 }}>
              <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--p4)', fontFamily: 'var(--f-mono)', letterSpacing: '.08em', marginBottom: 16 }}>
                  ...
                  return (
                    <div key={i} style={{ border: `1px solid ${color}30`, borderLeft: `3px solid ${color}`,
                      ...
                      <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 8, flexWrap: 'wrap' }}>
                        <span style={{ fontSize: 'var(--fs-xs)', background: `${color}20`, color, borderRadius: 4, padding: '2px 7px', fontFamily: 'var(--f-mono)', textTransform: 'uppercase' }}>{crit}</span>
                        {b.category && <span style={{ fontSize: 'var(--fs-xs)', color: 'var(--p5)', fontFamily: 'var(--f-mono)' }}>{b.category}</span>}
                        {b.change_type && <span style={{ fontSize: 'var(--fs-xs)', color: 'var(--p4)' }}>{b.change_type}</span>}
                      </div>
                      {b.original && (
                        <div style={{ fontSize: 'var(--fs-sm)', color: 'var(--cr5)', background: 'rgba(185,28,28,0.06)',
                          borderRadius: 4, padding: '6px 10px', marginBottom: 6, fontFamily: 'monospace', whiteSpace: 'pre-wrap' }}>
                          − {b.original}
                        </div>
                      )}
                      {b.revised && (
                        <div style={{ fontSize: 'var(--fs-sm)', color: 'var(--jade2)', background: 'rgba(16,185,129,0.06)',
                          borderRadius: 4, padding: '6px 10px', fontFamily: 'monospace', whiteSpace: 'pre-wrap' }}>
                          + {b.revised}
                        </div>
                      )}
                      {b.legal_context && (
                        <p style={{ fontSize: 'var(--fs-xs)', color: 'var(--p4)', marginTop: 8, lineHeight: 1.5 }}>{b.legal_context}</p>
                      )}
                    </div>
                  );
                })}
      ...
      {htmlDiff && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', zIndex: 1000,
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center',
            ...
            <span style={{ fontFamily: 'var(--f-mono)', fontSize: 'var(--fs-sm)', color: 'var(--p3)' }}>DIFF VISUAL</span>
